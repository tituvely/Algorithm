...
public class SW_test2 {
    ...
    public static void findMinValue() {
        ...
        // 둘째줄
        // dp[n][m] = 왼쪽에서 올 때, 오른쪽에서 올 때, 위에서 올 때, 아래에서 올 때의 최댓값 + weight[n][m]
        ...
        for(int i = 2; i <= N; i++) {
            ...
            left[0] = dp[i-1][1];
            for(int j = 1 ; j <= M; j++) {
                left[j] = Math.min(left[j-1], dp[i-1][j]) + weight[i][j];
            }
            ...
            right[M+1] = dp[i-1][M];
            for(int j = M; j >=1 ; j--) {
                right[j] = Math.min(right[j+1], dp[i-1][j]) + weight[i][j]; 
            }

            //left[i]와 right[i] 중에 더 큰 값은 왼쪽, 오른쪽, 위쪽에서 오는 모든 경우에 대한 최댓값이 된다
            for(int j = 1; j <= M; j++) {
                dp[i][j] = Math.min(left[j], right[j]);
            }
        }
